...
def generate_controllers_with_key_combinations(series, key_dict_list, output_folder_path, scone_base_files_path,
                                                 main_file_name):
    '''Takes as input a list of dictionnaries, each one defining multiple values
    of a unique parameter and combines them together to generate the desired folder
    structure, necessary to run SCONE simulations for each combination of values.
    key_dict_list is a list of dictionaries, each one defining a parameters for two targets
    ...
    ----------
    series: (PANDAS Series) recursive series containing controller data.
    key_dict_list: (list) list of dictionaries, each one defining a parameter.
