...
min_max_ratio = 1  # new max/min = value*(1 +/- min_max_ratio)
std_ratio = 0.1  # new std = value*std_ratio

series_new_range = ptc.apply_par_file_to_series_with_new_range_2(par_file, series, min_max_ratio, std_ratio)
...
if param_1 not in ['max_isometric_force', 'optimal_fiber_length']:
    ...
    if case == '1D':
        ...
        output_folder_path = '../new_exports/1D_std/' + phase_1 + '/' + 'stance_' + param_1
    ...
    scone_base_files_path = '../new_models/scone_base_files'
...
# generate scone main files with altered biomechanical parameters
    ...
    if case == '1D':
        key_dict_list = [key_dict_1]
        output_folder_path = '../new_exports/nostd_1D/biomechanical/' + param_1
